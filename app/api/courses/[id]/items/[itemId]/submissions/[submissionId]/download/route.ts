...
export async function GET(
  ...
  {
    params,
  }: { params: Promise<{ id: string; itemId: string; submissionId: string }> }
) {
  try {
    const session = requireSession(request);
    const { id: courseId, itemId, submissionId } = await params;
    ...
    const subSnap = await get(
      ref(database, `course_item_submissions/${courseId}/${itemId}/${submissionId}`)
    );
    if (!subSnap.exists()) return NextResponse.json({ error: 'Submission tidak ditemukan' }, { status: 404 });

    const submission = subSnap.val() as any;
    if (!access.isOwner && submission?.createdBy !== session.userId) {
      return NextResponse.json({ error: 'FORBIDDEN' }, { status: 403 });
    }

    const fileUrl = typeof submission?.fileUrl === 'string' ? submission.fileUrl : '';
    if (!fileUrl) return NextResponse.json({ error: 'File tidak tersedia' }, { status: 404 });
    ...
    const filename = safeFileName(submission?.fileName, `tugas-${submissionId}`);
