...
export async function GET(
  ...
  try {
    ...
    const database = getRtdb();
    const snapshot = await get(ref(database, `user_attendance_attempts/${session.userId}/${courseId}`));
    const attemptsByAbsensiId: Record<string, any> = {};
    if (snapshot.exists()) {
      snapshot.forEach((child) => {
        if (!child.key) return;
        attemptsByAbsensiId[child.key] = child.val();
      });
    }

    return NextResponse.json({ ok: true, attemptsByAbsensiId });
  } catch (error: any) {
